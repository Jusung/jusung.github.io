...
## Subscribe의 정의 
이럴 때는 공식 문서를 다시한번 확인해 보는게 좋을 것 같습니다. 공식 문서를 한번 살펴보겠습니다. 설명의 처음 한 문단만 보겠습니다.
...
역시 공식문서느님은 말해주고 계셨네요. 문서를 읽으며 새로 알게된 사실은 두가지 입니다.

하나는 `Subscribe`가 `Observable`에 `Observer`를 연결해주는구나!
...
헐… 보시다시피 `Subscribe`안에서 `Observer`를 생성하고 있네요! `Observer`는 `event`를 종류별로 방출하는 역할을 하고 있네요. 말그대로 이벤트를 관찰하고 거기에 맞는 행위를 하는 `Observer`역할을 하는 진짜 `Observer`입니다.
...
![Subscribe]({{ site.baseurl }}/images/2019/RxSwift - Subscribe.png)

$$그림 하나 넣읍시다.$$

`Subscribe`가 하는 일을 기억해두세요. 사실 이번 포스트는 다음 포스트인 `Observable`의 `Share`와 `Share(replay)`를 설명하기 위한 사전 포스트거든요. 😊

다음 포스트에서 또 만나요~ 
RxSwift는 아직도 다 모르겠네요. 😅

[foot notes]
1] Subscribe:  [ReactiveX - Subscribe operator](http://reactivex.io/documentation/operators/subscribe.html)